...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Post.device'
        db.add_column('pin_post', 'device',
                      self.gf('django.db.models.fields.IntegerField')(default=1, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'Stream', fields ['following', 'post', 'user']
        db.create_unique('pin_stream', ['following_id', 'post_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Stream', fields ['following', 'post', 'user']
        db.delete_unique('pin_stream', ['following_id', 'post_id', 'user_id'])

        # Deleting field 'Post.device'
        db.delete_column('pin_post', 'device')
