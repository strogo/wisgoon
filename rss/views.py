...
from rss.sphinxapi import SphinxClient, SPH_MATCH_EXTENDED,\
    SPH_SORT_ATTR_DESC, SPH_MATCH_ANY, SPH_GROUPBY_DAY
...
def older(request):
    ...
    if id != 0:
        latest_items = Item.objects.all().extra(where=['id<%s'], params=[id])\
            .order_by('-id')[:MAX_PER_PAGE]

        if request.is_ajax():
            if latest_items.exists():
                return render(request, 'rss/_items.html',
...
def home(request):
    ...
    if timestamp == 0:
        ...
        latest_items = Item.objects.all()\
            .extra(where=['id<%s'], params=[int(timestamp)])\
            .order_by('-id')[:MAX_PER_PAGE]
...
def user_likes(request, user_id):
    ...
    if offset > 0:
        likes = Likes.objects.filter(user=user_id).all()\
            .order_by('-id')[offset:offset+30]
    else:
        likes = Likes.objects.filter(user=user_id).all().order_by('-id')[:30]
    lss = []
    for l in likes:
        lss.append(l.item_id)
    latest_items = Item.objects.filter(id__in=lss).all()\
...
def feed(request, feed_id, older=0):
    ...
    if timestamp == 0:
        latest_items = Item.objects.filter(feed=feed_id).all()\
            .order_by('-id')[:MAX_PER_PAGE]
    else:
        latest_items = Item.objects\
            .filter(feed=feed_id)\
            .extra(where=['id<%s'], params=[timestamp])\
            .all()\
            .order_by('-id')[:MAX_PER_PAGE]
...
def subscribe(request):
    ...
    return render(request, 'rss/subscribe.html',{'form': form})
    ...
    if request.is_ajax() and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if created:
                ...
                if feed_status == 1:
                    sub, sub_created = Subscribe.objects.\
                        get_or_create(user=request.user, feed=feed)
...
def store_extra(item_id, tag, time):
    ...
    if not upex:
        ...
        upex = ItemExtra.objects(item_id=item_id, tags__nin=[tag])\
...
def lastview(request):
    ...
    return render(request, 'rss/lastview.html', {'latest_items': result})
...
def search(request):
    ...
    if q != '':
        ...
        if request.is_ajax():
            ...
            return render(request, 'rss/search.html',
                          {'latest_items': result,
                           'offset': offset + 30,
                           'q': q,
                           'sort': sort,
        ...
        return render(request, 'rss/tags.html', {'sobj': sObj})
...
def tag(request, q, older=0):
    if q != '':
        ...
        if request.is_ajax():
            return render(request, 'rss/_tag_items.html',
                          {'latest_items': result,
                           'offset': offset + 30,
                           'q': q,
                           'older_url': older_url})
            ...
            return render(render, 'rss/tagview.html',
                          {'latest_items': result,
                           'offset': offset + 30,
                           'q': q,
                           'older_url': older_url})
...
def comment_posted(request):
    if request.GET['c']:
        ...
        entry = Item.objects.get(id=comment.object_pk)
        if entry:
...
def report(request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                item = Item.objects.get(pk=item_id)
                reported = Report.objects.filter(user=request.user, item=item)\
                    .count()
